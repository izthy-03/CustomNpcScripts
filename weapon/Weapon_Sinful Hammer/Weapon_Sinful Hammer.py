...
class Entity:
    def __init__(self, _char):
        ...
        self.SkillTree.regSkill("Prepare", self.prepare(), self.isusing, lambda: 0, lambda: 1)
        self.SkillTree.regSkill("Smite", self.smite(), lambda: True, lambda: 0, lambda: 0,"Prepare")
        self.SkillTree.regSkill("Cooldown", self.toZero(), lambda: True, lambda: 2, lambda: 0, "Smite")
    ...
    def smite(self):
        for entity in self.char.player.world.getNearbyEntities(self.char.player.pos, 9, 1000):
